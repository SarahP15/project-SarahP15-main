...
Scene boat() {
	...
	// boat
	auto boat = assimpLoad("models/boat2/boat.obj", true);

	// water
	std::vector<Texture> textures = {
		loadTexture("models/waterTexture.png", "baseTexture"),
	};
	auto mesh1 = Mesh3D::square(textures);
	auto water = Object3D(std::vector<Mesh3D>{mesh1});
	water.grow(glm::vec3(100, 100, 100));
	water.rotate(glm::vec3(-M_PI / 2, 0, 0));

	// sky
	std::vector<Texture> textures2 = {
		loadTexture("models/skyTexture.png", "baseTexture"),
	};
	auto mesh2 = Mesh3D::square(textures2);
	auto sky = Object3D(std::vector<Mesh3D>{mesh2});
	sky.grow(glm::vec3(100, 100, 100));
	sky.move(glm::vec3(0, 45, -50));
	sky.rotate(glm::vec3(270, 0, 0));
	...
	while (running) {
		...
		while (window.pollEvent(ev)) {
			...
			if (ev.type == sf::Event::KeyPressed) {
				if (ev.key.code == sf::Keyboard::W && !isWPressed) {
					firstObject.setDirection(glm::vec3(0, 0, 1));
					isWPressed = true;
				}
				if (ev.key.code == sf::Keyboard::A && !isAPressed) {
					firstObject.setDirection(glm::vec3(1, 0, 0));
					isAPressed = true;
				}
				if (ev.key.code == sf::Keyboard::S && !isSPressed) {
					firstObject.setDirection(glm::vec3(0, 0, -1));
					isSPressed = true;
				}
				if (ev.key.code == sf::Keyboard::D && !isDPressed) {
					firstObject.setDirection(glm::vec3(-1, 0, 0));
					isDPressed = true;
				}
			}
			if (ev.type == sf::Event::KeyReleased) {
				if (ev.key.code == sf::Keyboard::W) isWPressed = false;
				if (ev.key.code == sf::Keyboard::A) isAPressed = false;
				if (ev.key.code == sf::Keyboard::S) isSPressed = false;
				if (ev.key.code == sf::Keyboard::D) isDPressed = false;
			}
		}

		if (sf::Keyboard::isKeyPressed(sf::Keyboard::W)) {
			firstObject.move(glm::vec3(0, 0, -boatSpeed));
		}
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::S)) {
			firstObject.move(glm::vec3(0, 0, boatSpeed));
		}
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::A)) {
			firstObject.move(glm::vec3(-boatSpeed, 0, 0));
		}
